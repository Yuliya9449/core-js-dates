...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(
    (Date.parse(dateEnd) - Date.parse(dateStart)) / millisecondsPerDay + 1
  );
}
...
function isDateInPeriod(date, { start, end }) {
  return (
    Date.parse(end) - Date.parse(date) >= 0 &&
    Date.parse(date) - Date.parse(start) >= 0
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    if (!((i + monthStartDay) % 7) || !((i + monthStartDay - 1) % 7)) {
      res += 1;
    }
  }
...
function getWeekNumberByDate(date) {
  const millisecondsPerDay = 24 * 60 * 60 * 1000;
  const weekLength = 7;
  const requiredDayIdx = 1;

  const firstJanuaryDate = new Date(date.getFullYear(), 0, 1);
  const firstJanuaryWeekDayIdx = firstJanuaryDate.getDay();

  const firstMondayDateInJanuary =
    (weekLength + requiredDayIdx - firstJanuaryWeekDayIdx) % weekLength;

  const countDaysFromFirstMondayDate =
    (date - firstJanuaryDate) / millisecondsPerDay -
    firstMondayDateInJanuary +
    1;

  const numberOfWeeksFromFirstMonday = Math.ceil(
    countDaysFromFirstMondayDate / 7
  );

  return (
    numberOfWeeksFromFirstMonday + (firstMondayDateInJanuary === 0 ? 0 : 1)
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let month = date.getMonth() + (date.getDate() < 13 ? 0 : 1);
  let nearestMonth13Date = new Date(year, month, 13);

  while (nearestMonth13Date.getDay() !== 5) {
    nearestMonth13Date = new Date(year, (month += 1), 13);
  }

  return nearestMonth13Date;
}
...
function getWorkSchedule({ start, end }, countWorkDays, countOffDays) {
  ...
  const currentDate = transformStringToDate(start);
  const endDate = transformStringToDate(end);
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      resArr.push(transformDateToString(currentDate));
